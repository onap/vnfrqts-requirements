...
.. req::
   ...
   A VNF or PNF producing VES events **MUST** should size the event buffer
...
.. req::
   ...
   **MUST** store in-scope events until the NFs event buffer (defined in
   R-636251) is full. Once buffer capacity is reached, then the oldest event in
   the buffer will be removed to make room for any new event (i.e. First In
   First Out)
