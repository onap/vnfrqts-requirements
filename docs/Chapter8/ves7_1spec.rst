...
**Example 2**: assumes an integer key for all events <n>: 000000001,
00000002, 000000003
...
* For each VES Listener destination, the NF must support two configurable
  endpoints. One will be the active, primary event listener endpoint.  The other
...
* If a NF is unable to establish a connection with a VES Event Listener or does
  ...
  connection/acknowledgement failure via the alternate FQDN/IP Address.
...
two values allowed for authentication method:
