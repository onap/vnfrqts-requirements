...
methods by configuring the VES Event Listener.

Basic authentication is supported in VES Event Listener for backward
compatibility for existing NFs that are already managed by ONAP. Going forward,
new NFs shall support Certificate Authentication. Because the security is
better, NFs may choose to only support Certificate Authentication and not
