...
.. req::
    ...
    Rationale: The proposed change focuses on the VNF providing security
    controls that enable the Operator using the VNF to implement access
    measure that enforce least privilege. It is not possible to test that
    the VNF provider has incorporated least privilege access measures in
    their development environment. The second sentence is a rationale,
